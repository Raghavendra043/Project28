...
import {update, getDocData, fileDetails, sendNotification, clientUpload} from '../../../../../firebasefunctions/firestore'
...
function Second({ formData, setForm, Loading }) {
    ...
    const uploadFile  = async(image)=>{
        ...
        const URL = await storage
            .ref(`${formData.clientEmail}/${image.name}`)
            .getDownloadURL()
        console.log(URL);        
        const file = await clientUpload('Projects', formData.title,image.name,URL);
        if(file && file === 1 ){Loading(false);}
        else if(file === 5){Loading(false);toast.error('Error occured', {position:"bottom-center"})}         
        sendNotification(2, formData.title, formData.currentStage, image.name);
      } catch(err){
        toast("Error Occured", {position:"bottom-center"})
        console.log(err);
      }
    }
